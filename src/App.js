...
const ProductItem = ({ product, onDelete, onUpdate }) => {
  ...
  const [editName, setIsEditName] = useState(product.name); //eslint-disable-line no-unused-vars
  const [editExplantion, setEditExplantion] = useState(product.explanation); //eslint-disable-line no-unused-vars
  const [editPrice, setEditPrice] = useState(product.price); //eslint-disable-line no-unused-vars
      ...
      {isEditMode && (
        ...
        >
          ...
          <input
            ...
            onChange={e => setEditExplantion(e.target.value)}
          ></input>
          <input
            type="number"
            placeholder="상품 가격"
